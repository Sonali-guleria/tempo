...
class AsOfJoinTest(SparkTest):
    ...
    def test_streaming_asofJoin(self):
        ...
        df_trades = self.spark.readStream.format("delta").table("left_ts_table").withColumn("event_ts", f.col("event_ts").cast("timestamp")).withWatermark("event_ts", "30 minutes")

        df_quotes = self.spark.readStream.format('delta').table("right_ts_table").withColumn("event_ts", f.col("event_ts").cast("timestamp")).withWatermark("event_ts", "30 minutes")
        left = TSDF(df_trades, partition_cols=['symbol'], ts_col='event_ts')
        right = TSDF(df_quotes, partition_cols=['symbol'], ts_col='event_ts')
        joined_df = left.asofJoin(right, interim_table='rp_interim_results3', options={'checkpointLocation': '/tmp/tempo/streaming_checkpoints/ii'})

        joined_df.df.writeStream("test").format("memory").outputMode("append").start()
        for x in range(5):
            import time
